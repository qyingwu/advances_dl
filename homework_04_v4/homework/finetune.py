...
def load_vlm(model_name: str = "vlm_model") -> BaseVLM:
    logging.info("Starting load_vlm function")
    try:
        # Try multiple possible locations for the model
        possible_paths = [
            Path(model_name),  # Current directory
            Path(__file__).parent / model_name,  # Next to this file
            Path(__file__).parent.parent / model_name,  # Root directory
        ]
        
        model_path = None
        for path in possible_paths:
            logging.info(f"Trying path: {path.absolute()}")
            if path.exists():
                logging.info(f"Found model at: {path.absolute()}")
                model_path = path
                break
        
        if model_path is None:
            logging.error("Could not find model in any expected location")
            raise FileNotFoundError("Model not found in any expected location")

        logging.info(f"Directory contents: {list(model_path.glob('*'))}")

        vlm = BaseVLM()
        logging.info("Created BaseVLM instance")

        vlm.model = PeftModel.from_pretrained(vlm.model, model_path).to(vlm.device)
        logging.info("Successfully loaded PeftModel")
        ...
        logging.error(f"Error in load_vlm: {str(e)}", exc_info=True)
        raise
...
def train(
    ...
    train_dataset_name: str = "train_grader",
